...
const EditJobPage = ({updateJobSubmit}) => {
  ...
  const [orgName, setOrgName] = useState(job.org.name);
  const [orgDescription, setOrgDescription] = useState(job.org.description);
  const [email, setEmail] = useState(job.org.email);
  const [phone, setPhone] = useState(job.org.phone);
  ...
  const submitForm = (e) => {
    ...
    toast.success('Updog successful! 🎊');  
  ...
  return (
    <section className="bg-summerBlush">
        ...
        >
            ...
            <div>
                <button
                className="bg-lightUnionRed hover:bg-unionRed text-cream font-bold py-2 px-4 rounded-full w-full focus:outline-none focus:shadow-outline mt-3"
                type="submit"
                >
                Update Job
                </button>
